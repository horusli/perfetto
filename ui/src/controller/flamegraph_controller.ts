...
export class FlamegraphController extends Controller<'main'> {
  ...
  run() {
    ...
    if (hasAreaChanged) {
      ...
      frontendGlobals.dispatch(Actions.openFlamegraph({
        upids,
        startPs: toPs(area.startSec),
        endPs: toPs(area.endSec),
  ...
  private async assembleFlamegraphDetails(
    ...
    const flamegraphMetadata = await this.getFlamegraphMetadata(
      selection.type,
      selectedFlamegraphState.startPs,
      selectedFlamegraphState.endPs,
    ...
    const key = `${selectedFlamegraphState.upids};${selectedFlamegraphState.startPs};${selectedFlamegraphState.endPs}`;
    ...
    try {
      ...
      if (flamegraphData !== undefined && selection &&
        ...
        selection.startPs === selectedFlamegraphState.startPs &&
        selection.endPs === selectedFlamegraphState.endPs) {
  ...
  private shouldRequestData(selection: FlamegraphState) {
    return selection.kind === 'FLAMEGRAPH_STATE' &&
      (this.lastSelectedFlamegraphState === undefined ||
        (this.lastSelectedFlamegraphState.startPs !== selection.startPs ||
          this.lastSelectedFlamegraphState.endPs !== selection.endPs ||
  ...
  async getFlamegraphData(
    baseKey: string, viewingOption: string, startPs: number, endPs: number,
    ...
    if (this.flamegraphDatasets.has(key)) {
      ...
      const tableName = await this.prepareViewsAndTables(
        startPs, endPs, upids, type, focusRegex);
  ...
  private async prepareViewsAndTables(
    startPs: number, endPs: number, upids: number[], type: ProfileType,
    ...
    if (type === ProfileType.PERF_SAMPLE) {
      ...
      return this.cache.getTableName(
        `select id, name, map_name, parent_id, depth, cumulative_size,
          ...
          where profile_type = '${flamegraphType}' and ${startPs} <= ts and
              ts <= ${endPs} and ${upidConditional}
  ...
  async getFlamegraphMetadata(
    type: ProfileType, startPs: number, endPs: number, upids: number[]) {
    // Don't do anything if selection of the marker stayed the same.
    if ((this.lastSelectedFlamegraphState !== undefined &&
      ((this.lastSelectedFlamegraphState.startPs === startPs &&
        this.lastSelectedFlamegraphState.endPs === endPs &&
        FlamegraphController.areArraysEqual(
    ...
    return { startPs, durPs: endPs - startPs, pids, upids, type };
  }
