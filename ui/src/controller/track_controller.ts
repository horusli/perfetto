...
export abstract class TrackController<
  ...
  cachedBucketSizePs(numRows: number): number | undefined {
    ...
    const traceDurPs = toPs(bounds.endSec - bounds.startSec);
    ...
    const outermostResolutionLevel =
      Math.ceil(Math.log2(traceDurPs / approxWidthPx));
    const outermostBucketPs = Math.pow(2, outermostResolutionLevel);
    ...
    // divide by.
    const cachedBucketSizePs = outermostBucketPs / bucketSubIntervals;

    // Our logic above should make sure this is an integer but double check that
    // here as an assertion before returning.
    assertTrue(Number.isInteger(cachedBucketSizePs));

    return cachedBucketSizePs;
  }
      ...
      if (this.requestingData) {
        ...
        promise
          .then(() => {
            ...
            if (Math.log2(toPs(resolution)) % 1 !== 0) {
              // resolution is in pixels per second so 1000 means
              // 1px = 1ms.
              resolution =
                fromPs(Math.pow(2, Math.floor(Math.log2(toPs(1000)))));
            }
